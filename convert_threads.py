...
for f in range(1, len(sys.argv)):
    ...
    try:
        with open('./data/raw/{0}'.format(sys.argv[f]), 'r') as csv_file:
    ...
    np.savetxt('./data/converted/{0}'.format(sys.argv[f]), output_arr, delimiter=',')
    print('Converted file saved at ./data/converted/{0}'.format(sys.argv[f]))